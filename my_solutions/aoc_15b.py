...
big_row = chitenz.copy()
biggest_row = big_row.copy()
for i in range(4):
    big_row+=1
    big_row[big_row>9]=1
    biggest_row = np.concatenate([biggest_row, big_row.copy()], axis=1)

big_chitenz = biggest_row.copy()
mega_chitenz = biggest_row.copy()
for c in range(4):
    big_chitenz+=1
    big_chitenz[big_chitenz>9]=1
    mega_chitenz = np.concatenate([mega_chitenz,big_chitenz.copy()], axis=0)

print(mega_chitenz.shape)

end = (mega_chitenz.shape[0]-1,mega_chitenz.shape[1]-1)
class Chiten():
    row_shift = [1, -1, 0, 0]
    col_shift = [0, 0, 1, -1]
...
my_lil_chiten = Chiten(start=(0,0), target=end, graph=mega_chitenz)
