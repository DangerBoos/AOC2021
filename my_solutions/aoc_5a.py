...
if example:
    ...
    dta = aocd.get_data(session=my_session, day=5, year=2021).splitlines()

direction_array = np.array([tuple(map(int, x.split(','))) for line in dta for x in line.split(' -> ')]).reshape(
    len(dta), 4)
...
for mask in x_mask, y_mask:
    ...
    for i in range(direction_array[mask, :].shape[0]):
        ...
        y_grid = [y_start] * len(list(np.arange(x_start, x_end + x_step, x_step))) if (
                y_start == y_end and x_start != x_end) else list(np.arange(y_start, y_end + y_step, y_step))
        x_grid = [x_start] * len(list(np.arange(y_start, y_end + y_step, y_step))) if (
                x_start == x_end and y_start != y_end) else list(np.arange(x_start, x_end + x_step, x_step))
